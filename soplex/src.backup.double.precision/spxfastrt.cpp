...
double SPxFastRT::minStability(double maxabs)
{
   if (maxabs < 1000.0)
      return minStab;
...
int SPxFastRT::maxDelta(
   double& val,                                /* on return: maximum step length */
   double& maxabs,                             /* on return: maximum absolute value in upd vector */
   UpdateVector& update,
...
SPxId SPxFastRT::maxDelta(
   int& nr,
   double& max,                                /* on return: maximum step length */
   double& maxabs)                             /* on return: maximum absolute value in delta vector */
{
...
bool SPxFastRT::maxShortLeave(double& sel, int leave, double maxabs)
{
   assert(leave >= 0);
   assert(maxabs >= 0);
...
bool SPxFastRT::minShortLeave(double& sel, int leave, double maxabs)
{
   assert(leave >= 0);
   assert(maxabs >= 0);
...
bool SPxFastRT::maxReLeave(double& sel, int leave, double maxabs)
{
...
bool SPxFastRT::minReLeave(double& sel, int leave, double maxabs)
{
   UpdateVector& vec = thesolver->fVec();
   Vector& low = thesolver->lbBound();
   Vector& up = thesolver->ubBound();
...
int SPxFastRT::selectLeave(double& val, double)
{
...
SPxId SPxFastRT::selectEnter(double& val, int)
{
   SPxId enterId;
   double max, sel;
   double maxabs = 0.0;
