...
/// Allocate memory for \p n nonzeros (specialization for double).
template<>
inline
void DSVectorBase<double>::allocMem(int n)
{
   spx_alloc(theelem, n);
   SVectorBase<double>::setMem(n, theelem);
}



/// Destructor (specialization for double).
template<>
inline
...
/// Reset nonzero memory to >= \p newmax.
/** This methods resets the memory consumption to \p newmax. However, if \p newmax < size(), it is
 *  reset to size() only (specialization for double).
 */
template<>
inline
void DSVectorBase<double>::setMax(int newmax)
{
   int siz = size();
   int len = (newmax < siz) ? siz : newmax;
