...
/// Resets \ref soplex::DVectorBase "DVectorBase"'s memory size to \p newsize (specialization for double).
template<>
inline
void DVectorBase<double>::reSize(int newsize)
{
   assert(newsize >= dim());
...
/// Default constructor with \p d as the initial dimension (specialization for double).
template<>
inline
DVectorBase<double>::DVectorBase(int d)
   : VectorBase<double>(0, 0)
   , mem(0)
{
   memsize = (d > 0) ? d : 4;
   spx_alloc(mem, memsize);
   ...
   assert(DVectorBase<double>::isConsistent());
}



/// Copy constructor (specialization for double).
template<>
template<>
inline
DVectorBase<double>::DVectorBase(const VectorBase<double>& old)
   : VectorBase<double>(0, 0)
   , mem( 0 )
{
   dimen = old.dim();
...
template<>
inline
DVectorBase<double>::DVectorBase(const DVectorBase<double>& old)
   : VectorBase<double>(0, 0)
   , mem(0)
{
   dimen = old.dim();
