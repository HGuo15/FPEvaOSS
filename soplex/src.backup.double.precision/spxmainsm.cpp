                                        ...
                                        DataArray<SPxSolver::VarStatus>& rStatus) const
{
   ...
   double newLo = (aij > 0) ? m_lhs/aij : m_rhs/aij;  // implicit lhs
   double newUp = (aij > 0) ? m_rhs/aij : m_lhs/aij;  // implicit rhs
                                           ...
                                           DataArray<SPxSolver::VarStatus>& rStatus) const
{
   ...
   for(int k = 0; k < m_row.size(); ++k)
   {
      ...
      double aij   = m_row.value(k);
      double oldLo = m_oldLowers[k];
      double oldUp = m_oldUppers[k];
      ...
      case SPxSolver::FIXED:
         if(m_fixed[k])
         {
            ...
            double violation = fabs(r[cIdx]/aij);
   ...
   if(cBasisCandidate >= 0)  // one of the variable in the row should in the basis
   {
      ...
      double aij = m_row.value(bas_k);
      double multiplier = r[cBasisCandidate]/aij;
                                               ...
                                               DataArray<SPxSolver::VarStatus>& rStatus) const
{
   ...
   if (m_loFree)
   {
	 ...
	 const SVector& row = m_rows[k];
         ...
         double scale = maxAbs(m_lRhs[k], val);

         if (scale < 1.0)
            scale = 1.0;

         double z = (m_lRhs[k] / scale) - (val / scale);

         if (isZero(z))
            z = 0.0;

         double up = z * scale / row[m_j];
	 slack.add(k, val);
                                               ...
                                               DataArray<SPxSolver::VarStatus>& rStatus) const
{
   ...
   double scale1 = maxAbs(m_lhs, s[m_i]);
   double scale2 = maxAbs(m_rhs, s[m_i]);
   ...
   double z1 = (m_lhs / scale1) - (s[m_i] / scale1);
   double z2 = (m_rhs / scale2) - (s[m_i] / scale2);
   ...
   double lo = (aij > 0) ? z1 * scale1 / aij : z2 * scale2 / aij;
   double up = (aij > 0) ? z2 * scale2 / aij : z1 * scale1 / aij;
                                            ...
                                            DataArray<SPxSolver::VarStatus>& rStatus) const
{
   ...
   double scale = maxAbs(m_lRhs, val);

   if (scale < 1.0)
      scale = 1.0;

   double z = (m_lRhs / scale) - (val / scale);
                                         ...
                                         DataArray<SPxSolver::VarStatus>& rStatus) const
{
   ...
   {
      double scale1 = maxAbs(x[m_k], m_loK);
      double scale2 = maxAbs(x[m_k], m_upK);
      ...
      double z1 = (x[m_k] / scale1) - (m_loK / scale1);
      double z2 = (x[m_k] / scale2) - (m_upK / scale2);
...
void SPxMainSM::handleExtremes(SPxLP& lp)
{
   ...
   double maxVal  = infinity / 5.0;
   double tol = feastol() * 1e-2;
...
SPxSimplifier::Result SPxMainSM::removeRowSingleton(SPxLP& lp, const SVector& row, int& i)
{
   ...
   double oldLo = lp.lower(j);
   double oldUp = lp.upper(j);
...
SPxSimplifier::Result SPxMainSM::simplifyRows(SPxLP& lp, bool& again)
{
   ...
   for(int i = lp.nRows()-1; i >= 0; --i)
   {
      ...
      double lhsBnd = 0.0; // minimal activity (finite summands)
      double rhsBnd = 0.0; // maximal activity (finite summands)
      ...
      if (rhsCnt <= 1 || lhsCnt <= 1)
      {
         for(int k = 0; k < row.size(); ++k)
         {
            ...
            if (aij > 0.0)
            {
               ...
               {
                  double lo    = -infinity;
                  double scale = maxAbs(lp.lhs(i), rhsBnd);

                  if (scale < 1.0)
                     scale = 1.0;

                  double z = (lp.lhs(i) / scale) - (rhsBnd / scale);
               ...
               {
                  double up    = infinity;
                  double scale = maxAbs(lp.rhs(i), lhsBnd);

                  if (scale < 1.0)
                     scale = 1.0;

                  double z = (lp.rhs(i) / scale) - (lhsBnd / scale);
      ...
      if (rhsCnt == 0 && EQrel(rhsBnd, lp.lhs(i), feastol()))
      {
         ...
         DataArray<double> lowers(row.size());
         DataArray<double> uppers(row.size());
	 ...
	 MSG_INFO3( spxout << "IMAISM30 col " << j
      ...
      if (col.size() == 1)
      {
                  ...
                  (lp.lower(j) > -infinity || lp.upper(j) < infinity))
         {
            ...
            double lo, up;
            double oldLower = lp.lower(k);
            double oldUpper = lp.upper(k);

            double scale1 = maxAbs(lhs, aij * lp.upper(j));
            double scale2 = maxAbs(lhs, aij * lp.lower(j));
            ...
            double z1 = (lhs / scale1) - (aij * lp.upper(j) / scale1);
            double z2 = (lhs / scale2) - (aij * lp.lower(j) / scale2);
         ...
         if (lp.lower(j) <= -infinity && lp.upper(j) >= infinity)
         {
            ...
            if (NErel(lp.lhs(i), lp.rhs(i), feastol()))
            {
               ...
               double sMaxObj = lp.maxObj(j) / aij; // after substituting variable j in objective
               double sLo     = lp.lhs(i);
               double sUp     = lp.rhs(i);
            ...
            for (int h = 0; h < row.size(); ++h)
            {
               ...
               if (k != j)
               {
                  double new_obj = lp.obj(k) - (lp.obj(j) * row.value(h) / aij);
...
SPxSimplifier::Result SPxMainSM::simplifyDual(SPxLP& lp, bool& again)
{
   ...
   for(int j = 0; j < lp.nCols(); ++j)
   {
      if (lp.colVector(j).size() == 1)
      {
         ...
         double aij = lp.colVector(j).value(0);

         ASSERT_WARN( "WMAISM44", isNotZero(aij, epsZero()) );

         double bound = lp.maxObj(j) / aij;
   ...
   for(int k = 0; k < dupRows.size(); ++k)
   {
      if (dupRows[k].size() > 1 && !(lp.rowVector(dupRows[k].index(0)).size() == 1))
      {
         ...
         for(int l = 0; l < dupRows[k].size(); ++l)
         {
            ...
            {
               double scaledLhs, scaledRhs;
               double factor = scale[rowIdx] / scale[i];
         ...
         if (rowIdx != -1)
         {
            double newLhs = (maxLhs > lp.lhs(rowIdx)) ? maxLhs : lp.lhs(rowIdx);
            double newRhs = (minRhs < lp.rhs(rowIdx)) ? minRhs : lp.rhs(rowIdx);
      ...
      if (dupCols[k].size() > 1 && !(lp.colVector(dupCols[k].index(0)).size() == 1))
      {
         ...
         for(int l = 0; l < dupCols[k].size(); ++l)
         {
            for(int m = 0; m < dupCols[k].size(); ++m)
            {
               ...
               {
                  double cj1 = lp.maxObj(j1);
                  double cj2 = lp.maxObj(j2);

                  // A.j1 = factor * A.j2
                  double factor = scale[j1] / scale[j2];
                  double objDif = cj1 - cj2 * scale[j1] / scale[j2];
...
void SPxMainSM::fixColumn(SPxLP& lp, int j, bool correctIdx)
{
   ...
   if (isNotZero(lo, epsZero()))
   {
      for(int k = 0; k < col.size(); ++k)
      {
         ...
         if (lp.rhs(i) < infinity)
         {
            double y     = lo * col.value(k);
            double scale = maxAbs(lp.rhs(i), y);

            if (scale < 1.0)
               scale = 1.0;

            double rhs = (lp.rhs(i) / scale) - (y / scale);
         ...
         {
            double y     = lo * col.value(k);
            double scale = maxAbs(lp.lhs(i), y);

            if (scale < 1.0)
               scale = 1.0;

            double lhs = (lp.lhs(i) / scale) - (y / scale);
...
SPxSimplifier::Result SPxMainSM::simplify(SPxLP& lp, double eps, double ftol, double otol, bool keepbounds)
{
