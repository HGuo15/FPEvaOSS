 ...
 */
double SPxSolver::test(int i, SPxBasis::Desc::Status stat) const
{
   assert(type() == ENTER);
   assert(!isBasic(stat));
...
double SPxSolver::computePvec(int i)
{

   return (*thePvec)[i] = vector(i) * (*theCoPvec);
}

double SPxSolver::computeTest(int i)
{
 ...
 */
double SPxSolver::coTest(int i, SPxBasis::Desc::Status stat) const
{
   assert(type() == ENTER);
   assert(!isBasic(stat));
