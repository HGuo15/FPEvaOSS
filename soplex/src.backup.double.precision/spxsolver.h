...
private:
   ...
   double           theCumulativeTime; ///< cumulative time spent in all calls to method solve()
   ...
   double           objLimit;    ///< objective value limit.
   Status         m_status;    ///< status of algorithm.

   double           m_entertol;  ///< feasibility tolerance maintained during entering algorithm
   double           m_leavetol;  ///< feasibility tolerance maintained during leaving algorithm
   double           theShift;    ///< sum of all shifts applied to any bound.
   ...
   bool           instableEnter;
   double           instableEnterVal;

   int            displayFreq;
   double           sparsePricingFactor; ///< enable sparse pricing when viols < factor * dim()
...
protected:
   ...
   virtual bool precisionReached(double& newpricertol) const;
   //@}
...
public:
   ...
   double provedBound(Vector& dualsol, const Vector& objvec) const;

   /// proved dual bound for objective value.
   virtual double provedDualbound() const;
   ...
   void setFeastol(double d);
   /// set parameter \p opttol.
   void setOpttol(double d);
   ...
   void setSparsePricingFactor(double fac)
   {
...
public:
   ...
   ///
   virtual void changeObj(int i, const double& newVal);
   ///
   virtual void changeObj(SPxColId p_id, const double& p_newVal)
   {
   ...
   ///
   virtual void changeLower(int i, const double& newLower);
   ///
   virtual void changeLower(SPxColId p_id, const double& p_newLower)
   {
   ...
   ///
   virtual void changeUpper(int i, const double& newUpper);
   ///
   virtual void changeUpper(SPxColId p_id, const double& p_newUpper)
   {
   ...
   ///
   virtual void changeBounds(int i, const double& newLower, const double& newUpper);
   ///
   virtual void changeBounds(
      SPxColId p_id, const double& p_newLower, const double& p_newUpper)
   {
   ...
   ///
   virtual void changeLhs(int i, const double& newLhs);
   ///
   virtual void changeLhs(SPxRowId p_id, const double& p_newLhs)
   {
   ...
   ///
   virtual void changeRhs(int i, const double& newRhs);
   ///
   virtual void changeRhs(SPxRowId p_id, const double& p_newRhs)
   {
   ...
   ///
   virtual void changeRange(int i, const double& newLhs, const double& newRhs);
   ///
   virtual void changeRange(
      SPxRowId p_id, const double& p_newLhs, const double& p_newRhs)
   {
   ...
   ///
   virtual void changeElement(int i, int j, const double& val);
   ///
   virtual void changeElement(
      SPxRowId rid, SPxColId cid, const double& val)
   {
   ...
   virtual void qualConstraintViolation(double& maxviol, double& sumviol) const;
   /// get violations of bounds.
   virtual void qualBoundViolation(double& maxviol, double& sumviol) const;
   /// get the residuum |Ax-b|.
   virtual void qualSlackViolation(double& maxviol, double& sumviol) const;
   /// get violation of optimality criterion.
   virtual void qualRedCostViolation(double& maxviol, double& sumviol) const;
   //@}
...
private:
   ...
   ///
   void perturbMin(
      const UpdateVector& vec, Vector& low, Vector& up, double eps, double delta,
...
private:
   ...
   double coTest(int i, SPxBasis::Desc::Status stat) const;
...
protected:
   ...
   ///
   virtual void getLeaveVals2(double leaveMax, SPxId enterId,
      double& enterBound, double& newUBbound,
      double& newLBbound, double& newCoPrhs);
   ///
   virtual void getEnterVals(SPxId id, double& enterTest,
   ...
   //@{
   ///
   void clearDualBounds(SPxBasis::Desc::Status, double&, double&) const;
   ///
...
public:
   ...
   Status 
   getResult( double* value = 0, Vector* primal = 0,
