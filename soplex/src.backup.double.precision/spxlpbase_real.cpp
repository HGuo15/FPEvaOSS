 ...
 * @brief Saving LPs with double values in a form suitable for SoPlex.
 */
 ...
 *  numbers like .e10 !
 */
static double LPFreadValue(char*& pos)
{
 ...
 */
static int LPFreadColName(char*& pos, NameSet* colnames, LPColSetBase<double>& colset, const LPColBase<double>* emptycol)
{
...
static double LPFreadInfinity(char*& pos)
{
   ...
   DIdxSet*      p_intvars)              ///< integer variables.
{
   ...
   LPColSetBase<double> cset;              ///< the set of columns read.
   LPRowSetBase<double> rset;              ///< the set of rows read.
   LPColBase<double> emptycol;             ///< reusable empty column.
   LPRowBase<double> row;                  ///< last assembled row.
   DSVectorBase<double> vec;               ///< last assembled vector (from row).
   ...
   SPxLPBase<double>::clear(); // clear the LP.
   ...
   for(;;)
   {
      ...
      if( section == START )
      {
         ...
         {
            changeSense(SPxLPBase<double>::MAXIMIZE);
            section = OBJECTIVE;
         }
         else if( LPFhasKeyword(pos, "min[imize]") )
         {
            changeSense(SPxLPBase<double>::MINIMIZE);
            section = OBJECTIVE;
         }
      }
...
static void MPSreadRows(MPSInput& mps, LPRowSetBase<double>& rset, NameSet& rnames)
{
...
static void MPSreadCols(MPSInput& mps, const LPRowSetBase<double>& rset, const NameSet&  rnames, LPColSetBase<double>& cset, NameSet& cnames, DIdxSet* intvars)
{
   double val;
   int idx;
   char colname[MPSInput::MAX_LINE_LEN] = { '\0' };
   LPColBase<double> col(rset.num());
...
/// Process RHS section.
static void MPSreadRhs(MPSInput& mps, LPRowSetBase<double>& rset, const NameSet& rnames)
{
...
static void MPSreadRanges(MPSInput& mps,  LPRowSetBase<double>& rset, const NameSet& rnames)
{
...
static void MPSreadBounds(MPSInput& mps, LPColSetBase<double>& cset, const NameSet& cnames, DIdxSet* intvars)
{
...
template <>
bool SPxLPBase<double>::readMPS(
   std::istream& p_input,           ///< input stream.
   NameSet*      p_rnames,          ///< row names.
   NameSet*      p_cnames,          ///< column names.
   DIdxSet*      p_intvars)         ///< integer variables.
{
   LPRowSetBase<double>& rset = *this;
   LPColSetBase<double>& cset = *this;
   NameSet* rnames;
   NameSet* cnames;
   ...
   {
      changeSense(mps.objSense() == MPSInput::MINIMIZE ? SPxLPBase<double>::MINIMIZE : SPxLPBase<double>::MAXIMIZE);

      MSG_INFO2( spxout << "IMPSRD06 Objective sense: " << ((mps.objSense() == MPSInput::MINIMIZE) ? "Minimize\n" : "Maximize\n") );

      added2Set(
         *(reinterpret_cast<SVSetBase<double>*>(static_cast<LPRowSetBase<double>*>(this))),
         *(reinterpret_cast<SVSetBase<double>*>(static_cast<LPColSetBase<double>*>(this))),
         cset.num());
      addedCols(cset.num());
...
static void LPFwriteSVector(
   const SPxLPBase<double>&   p_lp,       ///< the LP
   std::ostream&            p_output,   ///< output stream
   const NameSet*           p_cnames,   ///< column names
   const SVectorBase<double>& p_svec )    ///< vector to write
{
   ...
   )
{
   ...
   p_output << ((sense == SPxLPBase<double>::MINIMIZE) ? "Minimize\n" : "Maximize\n");
   p_output << "  obj: ";

   const VectorBase<double>& obj = p_lp.maxObj();
   DSVectorBase<double> svec(obj.dim());
   svec.operator=(obj);
   svec *= Real(sense);
...
static void LPFwriteRow(
   ...
   std::ostream&            p_output,   ///< output stream
   const NameSet*           p_cnames,   ///< column names
   const SVectorBase<double>& p_svec,     ///< vector of the row
   const double&              p_lhs,      ///< lhs of the row
   const double&              p_rhs       ///< rhs of the row
   )
{
   ...
   )
{
   ...
   {
      const double lhs = p_lp.lhs(i);
      const double rhs = p_lp.rhs(i);
...
static void LPFwriteBounds(
   const SPxLPBase<double>&   p_lp,       ///< the LP to write
   std::ostream&            p_output,   ///< output stream
   const NameSet*           p_cnames    ///< column names
   )
{
   ...
   for( int j = 0; j < p_lp.nCols(); ++j )
   {
      const double lower = p_lp.lower(j);
      const double upper = p_lp.upper(j);
...
template <>
void SPxLPBase<double>::writeLPF(
   std::ostream&  p_output,          ///< output stream
   const NameSet* p_rnames,          ///< row names
   const NameSet* p_cnames,          ///< column names
   const DIdxSet* p_intvars          ///< integer variables
   ...
   )
{
   ...
   if( name1 != 0 )
   {
      sprintf(buf, "  %-8.8s  %.15" "f", name1, value1);
      os << buf;

      if( name2 != 0 )
      {
         sprintf(buf, "   %-8.8s  %.15" "f", name2, value2);
         os << buf;
      }
   }
...
static double MPSgetRHS(double left, double right)
{
   double rhsval;
 ...
 */
template <>
void SPxLPBase<double>::writeMPS(
   std::ostream&  p_output,          ///< output stream.
   const NameSet* p_rnames,          ///< row names.
   const NameSet* p_cnames,          ///< column names.
   const DIdxSet* p_intvars          ///< integer variables.
   ) const
{
   ...
   for( int j = 0; j < (has_intvars ? 2 : 1); j++ )
   {
      ...
      for( i = 0; i < nCols(); i++ )
      {
         ...
         const SVectorBase<double>& col = colVector(i);
   ...
   if( spxSense() == SPxLPBase<double>::MAXIMIZE )
   {
...
template class SPxLPBase < double >;
