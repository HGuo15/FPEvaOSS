...
private:
   ...
   //@{
   ///
   double degenerateEps() const;

   ///
   int maxDelta(
      double* /*max*/,        ///< max abs value in \p upd
      double* val,            ///< initial and chosen value
      int num,              ///< number of indices in \p idx
      const int* idx,       ///< nonzero indices in \p upd
      const double* upd,      ///< update vector for \p vec
      const double* vec,      ///< current vector
      const double* low,      ///< lower bounds for \p vec
      const double* up,       ///< upper bounds for \p vec
