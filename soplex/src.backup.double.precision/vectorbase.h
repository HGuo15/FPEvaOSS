...
/// Assignment operator (specialization for double).
template <>
inline
VectorBase<double>& VectorBase<double>::operator=(const VectorBase<double>& vec)
{
   if( this != &vec )
   {
      assert(dim() == vec.dim());

      memcpy(val, vec.val, (unsigned int)dimen*sizeof(double));
...
/// Set vector to 0 (specialization for double).
template<>
inline
void VectorBase<double>::clear()
{
   if( dimen > 0 )
      memset(val, 0, (unsigned int)dimen * sizeof(double));
}
