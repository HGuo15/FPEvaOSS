...
public:
   ...
   virtual Result simplify(SPxLP& lp, double eps, double delta) = 0;
   /// simplify SPxLP \p lp with independent primal and dual feasibility tolerance.
   virtual Result simplify(SPxLP& lp, double eps, double feastol, double opttol, bool keepbounds = false) = 0;
   ...
   virtual double getObjoffset() const
   {
      return m_objoffset;
   }

   /// add objective offset.
   virtual void addObjoffset(const double val)
   {
      m_objoffset += val;
   }
   //@}
