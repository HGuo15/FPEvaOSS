 ...
 */
double SPxHarrisRT::degenerateEps() const
{
   return solver()->delta() 
...
int SPxHarrisRT::maxDelta(
   ...
   double epsilon)  const       /* what is 0? */
{
   double x;
   double theval;
 ...
 */
int SPxHarrisRT::selectLeave(double& val, double)
{
   int i, j;
   double stab, x, y;
   double max;
...
SPxId SPxHarrisRT::selectEnter(double& val, int)
{
   int i, j;
   SPxId enterId;
   double stab, x, y;
   ...
   if (val > epsilon)
   {
      for(;;)
      {
         ...
         for (j = pupd.size() - 1; j >= 0; --j)
         {
            ...
            {
               MSG_DEBUG( spxout << "DHARRI01 removing value " << pupd[i] << std::endl; )
               pupd.clearNum(j);
            }
         }
         for (j = cupd.size() - 1; j >= 0; --j)
         {
            ...
            {
               MSG_DEBUG( spxout << "DHARRI02 removing value " << cupd[i] << std::endl; )
               cupd.clearNum(j);
            }
         }
   ...
   else if (val < -epsilon)
   {
      for(;;)
      {
         ...
         for (j = pupd.size() - 1; j >= 0; --j)
         {
            ...
            {
               MSG_DEBUG( spxout << "DHARRI03 removing value " << pupd[i] << std::endl; )
               pupd.clearNum(j);
            }
         }
         for (j = cupd.size() - 1; j >= 0; --j)
         {
            ...
            {
               MSG_DEBUG( spxout << "DHARRI04 removing value " << x << std::endl; );
               cupd.clearNum(j);
            }
         }
